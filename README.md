...
- 顶部可开启 Solo 模式；开启后“让 AI 生成建议”会直接进入 Agent 会话执行流。
- 默认开启写文件、关闭终端，且保留人工审批，适合单人持续开发。
- 支持配置 Solo 自动推进步数，并自动切换到 Agent 面板观察执行轨迹。


## IDE 设置总览（新增）

- 设置面板新增分组（通用 / 编辑器 / AI），更接近 Trae 的设置结构。
- 通用：UI 密度、自动保存、行号显示。
- 编辑器：字体大小、缩进空格、自动换行。
- AI：Temperature、系统提示词。
- 设置项持久化到本地，启动后自动恢复。
